    ...
    **kwargs,
    ...
    def register_recr(
        ...
        if net.__class__.__name__ == filter_name:
            net.forward = warp_custom(net, gate_step=gate_step, inference_num_per_image=inference_num_per_image, lcm=lcm)
            return count + 1
        elif hasattr(net, 'children'):
            for net_child in net.children():
                count = register_recr(net_child, count, gate_step=gate_step, inference_num_per_image=inference_num_per_image, lcm=lcm)
        return count
...
class PixArtAlphaPipeline(DiffusionPipeline):
    ...
    def tgate(
        ...
        **kwargs,
        ...
        # 8. Initializing the TGATE configuration. Detailed information in https://github.com/HaozheLiu-ST/T-GATE
        register_tgate_forward(self.transformer, 
            'Attention',
            gate_step=gate_step,
            inference_num_per_image = num_inference_steps, 
            lcm= not do_classifier_free_guidance
            )
