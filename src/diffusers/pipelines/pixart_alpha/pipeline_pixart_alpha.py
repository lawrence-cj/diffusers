    ...
    **kwargs,
    ...
    def register_recr(
        ...
        if net.__class__.__name__ == filter_name:
            net.forward = warp_custom(net, gate_step=gate_step, inference_num_per_image=inference_num_per_image,
                                      lcm=lcm)
            return count + 1
        elif hasattr(net, 'children'):
            for net_child in net.children():
                count = register_recr(net_child, count, gate_step=gate_step,
                                      inference_num_per_image=inference_num_per_image, lcm=lcm)
        return count

    return register_recr(model, count, gate_step=gate_step, inference_num_per_image=inference_num_per_image, lcm=lcm)


def tgate_processor(
    attn: Optional[torch.FloatTensor] = None,
    ...
    **kwargs,
    ...
    if cross_attn and (gate_step < cur_step):
        ...
        if attn.spatial_norm is not None:
            hidden_states = attn.spatial_norm(hidden_states, t_emb)
...
class PixArtAlphaPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        do_tgate_accelerate = tgate_step > 1.0
        ...
        added_cond_nocfg_kwargs = {"resolution": None, "aspect_ratio": None}
        if self.transformer.config.sample_size == 128:
            resolution = torch.tensor([height, width]).repeat(batch_size * num_images_per_prompt, 1)
            aspect_ratio = torch.tensor([float(height / width)]).repeat(batch_size * num_images_per_prompt, 1)
            resolution = resolution.to(dtype=prompt_embeds.dtype, device=device)
            aspect_ratio = aspect_ratio.to(dtype=prompt_embeds.dtype, device=device)
            added_cond_nocfg_kwargs = {"resolution": resolution, "aspect_ratio": aspect_ratio}
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                if do_classifier_free_guidance and ((not do_tgate_accelerate) or i < tgate_step):
                    ...
                    added_cond_kwargs = added_cond_nocfg_kwargs
